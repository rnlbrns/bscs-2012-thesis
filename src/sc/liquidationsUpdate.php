...
class liquidationsUpdate extends konekServer
{
 function inputNewBill($BENEFICIARY_ID){
	...
	$query=mysql_query($queryText);
		...
		function Add()
		{
		  document.getElementById('PAYMENT_ALLOWANCE').value =
			parseFloat(document.getElementById('PAYMENT_BILL').value) -
			parseFloat(document.getElementById('PAYMENT_TUITION').value);
		}
		...
		$SYIhap++;
		$SYDisp=mysql_fetch_array($query);
		...
		<tr>
		<td><b>Amount:</b></td>
		<td><input type='text' id='PAYMENT_BILL' name='PAYMENT_BILL' value=".$fetchThis["GRANT_MERITDESCINT"]." readonly='readonly'/></td>
		</tr>
		<tr>
		<td><b>Tuition:</b></td>
		<td><input type='text' id='PAYMENT_TUITION' name='PAYMENT_TUITION' value='0' /></td>
		</tr><tr>
		<td><input type='button' value='Get Allowance' onclick='Add()'/></td>
		<td>
		
		<input type='text' id='PAYMENT_ALLOWANCE' name='PAYMENT_ALLOWANCE' value='' readonly='readonly'/></td>
		</tr>
		</table>
		<br>
...
function viewGrades($BENEFICIARY_ID){
		...
		<input type='submit' name='fw' value='Update Grade' />
		<input type='button' value='Cancel' onclick=parent.location='liquidationsMain.php'>
		<br />
		<br />";
...
echo "</select></td></tr><tr>
	...
	function updateGrade($GRADE_ID){
		$this->abriDB();
		
			$qryName=mysql_query("select * from grade_tbl a, beneficiary_tbl b where a.GRADE_ID=".$GRADE_ID." and a.BENEFICIARY_ID=b.BENEFICIARY_ID");	
					$qrow=mysql_fetch_array($qryName);
					$BENEFICIARY_ID=$qrow["BENEFICIARY_ID"];
					$BENEFICIARY_nam1=$qrow["BENEFICIARY_nam1"];
					$BENEFICIARY_nam2=$qrow["BENEFICIARY_nam2"];
...
$SYKawnt=mysql_num_rows($SYPagawasQry);
$SYIhap=0;
$disp=mysql_fetch_array(mysql_query("select * from grade_tbl where GRADE_ID=".$GRADE_ID));
while($SYIhap<$SYKawnt){
	$SYIhap++;
	$SYDisp=mysql_fetch_array($SYPagawasQry);
	$schoolyr_yr=$SYDisp["schoolyr_yr"];
...
<input type='hidden' value=".$GRADE_ID." name='GRADE_ID' />
		...
		<tr>
		<td><b>Subject</td><td><input type='text' name='GRADE_SUBJ' onKeyPress='return paraAddress(event, false)' style='width:200px' value='".$disp["GRADE_SUBJ"]."'/></td>
		</tr>
		<tr>
		<td><b>Units</td><td><input type='text' name='GRADE_UNIT' maxlength='5' onKeyPress='return numbersonly(event, false)' style='width:30px' value='".$disp["GRADE_UNIT"]."'/></td>
		</tr>
		<tr>
		<td><b>Grade</td><td><input type='text' name='GRADE_GRADE'  maxlength='5' onKeyPress='return numbersonly(event, false)' style='width:60px' value='".$disp["GRADE_GRADE"]."'/></td>
		</tr>
		<tr>
		<td><b>Catagory</b></td>
		<td>
		...
		<br />
		<input type='submit' name='fw' value='Submit Grade Update' />
		<input type='button' value='Cancel' onClick='history.back();'>
		</form></center>
		";
		}//end updateGrade
...
else if ($_POST["fw"]=="Submit Grade")
{
	...
	$inputGradeqry="INSERT INTO grade_tbl (GRADE_ID ,GRADE_SCHOOLYR ,GRADE_SEM ,GRADE_SUBJ ,GRADE_UNIT ,GRADE_GRADE ,BENEFICIARY_ID, schoolyr_yr, GRADE_REQ)
VALUES (NULL , ".$GRADE_SCHOOLYR.", ".$GRADE_SEM.", '".$GRADE_SUBJ."', ".$GRADE_UNIT.",".$GRADE_GRADE.", ".$BENEFICIARY_ID.", '".$schoolyr_yr."', '".$GRADE_REQ."'
)";
			
paragUpdateDB::updateTable($inputGradeqry);
$BENEFICIARY_STAT;
	if($GRADE_REQ=='Major' && $GRADE_GRADE>3){
		
		$query_recommendForDropping=mysql_query("update beneficiary_tbl set BENEFICIARY_WAIVECTR=BENEFICIARY_WAIVECTR+1 where BENEFICIARY_ID=".$BENEFICIARY_ID) or die(mysql_error());
		
		$queryselecttwofailmajorgrade=mysql_query("select * from beneficiary_tbl where BENEFICIARY_WAIVECTR=".$BENEFICIARY_ID);
		$queryfetch=mysql_fetch_array($queryselecttwofailmajorgrade);
		if($queryfetch["BENEFICIARY_WAIVECTR"]=2){
			$query_recommendForDroppingquery=mysql_query("update beneficiary_tbl set BENEFICIARY_STAT='Recommended for Waive' where BENEFICIARY_ID=".$BENEFICIARY_ID);
			}
		}

	$audittrailquery=mysql_query("insert into audittrail_tbl set audittrail_username='".$_SESSION["SC_USERNAM"]."',  audittrail_activity='Submitted A Grade of A Scholar', audittrail_time=now() ") or die(mysql_error());	
...
else if ($_POST["fw"]=="Submit Grade Update")
{
	...
	$checkIfSubExistQuery=mysql_query("select * from grade_tbl where GRADE_SUBJ='".$GRADE_SUBJ."' and  !(GRADE_ID=".$GRADE_ID.") and BENEFICIARY_ID=".$BENEFICIARY_ID);
	$checkIfSubExistNumRows=mysql_num_rows($checkIfSubExistQuery);

	if($checkIfSubExistNumRows==0){
	$inputGradeqry="update grade_tbl set GRADE_SCHOOLYR='".$GRADE_SCHOOLYR."', GRADE_SEM=".$GRADE_SEM.", GRADE_SUBJ='".$GRADE_SUBJ."',  schoolyr_yr='".$schoolyr_yr."',  GRADE_REQ='".$GRADE_REQ."', GRADE_GRADE=".$GRADE_GRADE.", GRADE_UNIT=".$GRADE_UNIT." where GRADE_ID=".$GRADE_ID;
...
else if ($_POST["fw"]=="Submit Bill")
{
	...
	$PAYMENT_TUITION=$_POST["PAYMENT_TUITION"];
	$PAYMENT_ALLOWANCE=$_POST["PAYMENT_ALLOWANCE"];
	$schoolyr_yr=$_POST["schoolyr_yr"];
...
VALUES (
NULL , ".$BENEFICIARY_ID.", ".$PAYMENT_BILL.",  ".$PAYMENT_TUITION.",  ".$PAYMENT_ALLOWANCE.", now(), '".$PAYMENT_SEM."', '".$schoolyr_yr."', 'UNPAID'
)";
	$queryInsertPaymentText=mysql_query($a) or die(mysql_error());
	$audittrailquery=mysql_query("insert into audittrail_tbl set audittrail_username='".$_SESSION["SC_USERNAM"]."',  audittrail_activity='Submitted A Bill of A Scholar', audittrail_time=now() ") or die(mysql_error());	
	
			echo "<script>alert('Bill Submitted!')</script>";
			echo "<script>window.location.href='liquidationsMain.php'</script>";

}
else if ($_POST["fw"]=="Update Grade")
{
	konekServer::abriDB();
	$GRADE_ID=$_POST["GRADE_ID"];
	if(empty($GRADE_ID))
	{
			...
			echo "<script>alert('Select a Grade to Update!')</script>";
			echo "<script>window.location.href='liquidationsMain.php'</script>";
	}
	...
	$sulod=new liquidationsUpdate();
	$butang=new paragLayOut();
	$butang->butangSulod($sulod->updateGrade($GRADE_ID));
