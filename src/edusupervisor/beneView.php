...
require_once("header.php");
	...
	konekServer::abriDB();
	$audittrailquery=mysql_query("insert into audittrail_tbl set audittrail_username='".$_SESSION["edusupervisorUsername"]."',  audittrail_activity='Viewed List of beneficiaries.', audittrail_time=now() ") or die(mysql_error());	
	
class liquidationsSulod extends konekServer{
	function liquidationsTbl(){
			$this->abriDB();
			$qry="select * from beneficiary_tbl a, grant_tbl b, program_tbl c, hei_tbl d where a.HEI_ID=".$_POST["HEI_ID"]." and d.HEI_ID=".$_POST["HEI_ID"]." and a.GRANT_ID=b.GRANT_ID and a.PROG_ID=c.PROG_ID and !(a.BENEFICIARY_STAT='WAIVED') order by BENEFICIARY_nam3 ASC";
			
			$qryPasa="select * from beneficiary_tbl a, grant_tbl b, program_tbl c, hei_tbl d where a.HEI_ID=".$_POST["HEI_ID"]." and d.HEI_ID=".$_POST["HEI_ID"]." and a.GRANT_ID=b.GRANT_ID and a.PROG_ID=c.PROG_ID and !(a.BENEFICIARY_STAT=&#34;WAIVED&#34;) order by BENEFICIARY_nam3 ASC";
			$kuery=mysql_query($qry);
			$kadamo=mysql_num_rows($kuery);
			$ihap=0;
			$skulnammongetqrytext="select * from hei_tbl where HEI_ID=".$_POST["HEI_ID"];
...
</form><center><b>LIST OF CHED STUDENT FINANCIAL ASSISTANCE PROGRAMS (STUFAPS) BENEFICIARIES</b>";
echo "<br />
As of ";
...
echo "<br />";
echo "
<form method='post' action='beneViewPDF.php'>
