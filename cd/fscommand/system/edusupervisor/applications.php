...
konekServer::abriDB();
	...
	echo "<form method='GET' action='applicationView.php'>";
...
<input type='submit' name='grantFW' value='View'/>
					...
					$listApp="SELECT * FROM app_tbl a, grant_tbl b WHERE a.APP_CONFREJ=0 and a.GRANT_ID=b.GRANT_ID and a.APP_DATEAPP  BETWEEN MAKEDATE( EXTRACT(YEAR FROM CURDATE()),1) AND MAKEDATE( EXTRACT(YEAR FROM CURDATE()),365) order by APP_ID DESC";
...
<td align='center'><b>App. Date</b></td>
</tr>";
if ($kadamo>0)
					{	
				while($ihap<$kadamo)
