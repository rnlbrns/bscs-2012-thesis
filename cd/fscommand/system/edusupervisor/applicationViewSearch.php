...
<form method='GET' action='appViewInfoApp.php'>";
					$listApp="SELECT * FROM app_tbl WHERE APP_LASTNAME='".$_GET["APP_LASTNAME"]."' and APP_CONFREJ=0 order by APP_ID DESC";
...
echo "<br />";
echo "
Search results  for: <b>".mb_strtoupper($_GET["APP_LASTNAME"])."</b>
...
if ($kadamo>0)
			...
			echo "	<tr>
				...
				";
								...
								if(!empty($_GET["APP_LASTNAME"]))
								{
								$sulod=new liquidationsSulod();
								$butang=new paragLayOut();
								$butang->butangSulod($sulod->liquidationsTbl());
								}
								else if(empty($_GET["APP_LASTNAME"])){
									
									echo "<script>alert('Please Input Last Name To Search!!');
